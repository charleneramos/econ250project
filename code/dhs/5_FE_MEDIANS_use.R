...
krdata_fe_amen <- krdata %>% 
  group_by(group2) %>%
  summarise(fe_amen = weighted.mean(fe_amen, na.rm=TRUE)) %>%  # wt
...
krdata_fe_abst <- krdata %>% 
  group_by(group2) %>%
  summarise(fe_abst = weighted.mean(fe_abst, na.rm=TRUE)) %>% # wt
  mutate(width = case_when(
    DHS_PHASE<=6 & group2==1 ~ 0.75,      
    DHS_PHASE<=6 & group2==2 ~ 1.5,      
    DHS_PHASE<=6 & group2==3 ~ 1.75,      
...
krdata_fe_insusc <- krdata %>% 
  mutate(fe_insusc = case_when(
    ...
    TRUE ~ 0)) %>%
  group_by(group2) %>%
  summarise(fe_insusc = weighted.mean(fe_insusc, na.rm=TRUE)) %>% # wt
  mutate(width = case_when(
    DHS_PHASE<=6 & group2==1 ~ 0.75,      
    DHS_PHASE<=6 & group2==2 ~ 1.5,      
    DHS_PHASE<=6 & group2==3 ~ 1.75,      
